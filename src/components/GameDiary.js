...
const DiaryTitle = () => {
  return (
    <div className="grid grid-cols-5 place-items-end">
      <span className="col-span-2 place-self-start font-semibold text-white">
        Game Diary
      </span>
...
const DiaryData = ({ theDay, date, played, won, points }) => {
  ...
  return (
    <div className="grid grid-cols-5">
      <span className="col-span-2 py-2 text-sm font-semibold text-white">
        {theDay === 'today'
          ? "Today"
          : theDay === 'yesterday'
          ? "Yesterday"
      ...
      {[played, won, points].map((value, index) => (
        <span
          key={index}
          className="flex items-center justify-end border-r border-gray-700 pr-2 text-sm text-white">
          {value}
        </span>
      ))}
    </div>
